...
let hotel = {
    ...
    bookSelectedRoom: function () {
        let selectedRoom = parseInt(
          document.getElementById('dropDownAvailable').value
        );
      ...
      unBookSelectedRoom: function () {
        let selectedRoom = parseInt(document.getElementById('dropDownBooked').value);
        // console.log(selectedRoom);
    ...
    updateAvailableRooms: function () {
        let dropDown = "<select id='dropDownAvailable'>"; // Check dropDownAvailable//
        for (let i = 0; i < this.availableRooms.length; i++) {
          for (let j = 0; j < this.availableRooms[i].length; j++) {
            dropDown =
              dropDown +
      ...
      updateBookedRooms: function () {
        let dropDown = "<select id='dropDownBooked'>";
        for (let i = 0; i < this.bookedRooms.length; i++) {
          for (let j = 0; j < this.bookedRooms[i].length; j++) {
            dropDown =
              dropDown +
